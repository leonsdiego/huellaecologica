...
class Model_Mapper_CuestionarioAlternativa
{
	...
	public function insertRespuesta($idCuestionario,$idAlternativa,$resultado,$idIndicador)
	{
		$fila = array(
	...
	*/
	...
	{
		$db = Zend_Db_Table::getDefaultAdapter();
		return $db->fetchAll($db->select()	->from(array('c'=>'tbl_cuestionario_has_tbl_alternativa'),
												array('resultado'=>'c.resultado_cuestionario_alternativa'))
											->where('c.tbl_cuestionario_id_cuestionario = ?',$idCuestionario)
											->join(array('i'=>'tbl_indicador'),'i.id_indicador = c.tbl_indicador_id_indicador')
											->where('i.cuenta_indicador = 1'));
	}
	...
	public function getResultadosByGrupo($idCuestionario,$idGrupo)
	{
		...
		return $db->fetchAll($db->select()	->from(array('ca'=>'tbl_cuestionario_has_tbl_alternativa'),array('resultado'=>'resultado_cuestionario_alternativa'))
											->where('ca.tbl_cuestionario_id_cuestionario = ?',$idCuestionario)
											->join(array('i'=>'tbl_indicador'),'i.id_indicador = ca.tbl_indicador_id_indicador')
											->where('i.cuenta_indicador = 1')
											->join(array('g'=>'tbl_grupo'),'g.id_grupo = i.tbl_grupo_id_grupo')
	...
	{
		$db = Zend_Db_Table::getDefaultAdapter();
		return $db->fetchAll($db->select()	->from(array('ca'=>'tbl_cuestionario_has_tbl_alternativa'))
											->where('ca.tbl_cuestionario_id_cuestionario = ?',$idCuestionario)
											->join(array('a'=>'tbl_alternativa'),'a.id_alternativa = ca.tbl_alternativa_id_alternativa')
											->join(array('c'=>'tbl_consejo'),'c.id_consejo = a.tbl_consejo_id_consejo')
											->where('c.id_consejo = a.tbl_consejo_id_consejo')
											->group('c.id_consejo')
